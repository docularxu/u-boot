...
/*
 * DDR firmware execution buffer in SRAM. Must lie strictly between
 * the end of SPL TEXT (~0xc0815xxx, see spl/u-boot-spl.bin size) and
 * the bottom of SPL's early malloc heap (= SPL_STACK - SPL_SYS_MALLOC_F_LEN
 * = 0xc0830000 with the current 64 KiB heap). 0xc0820000 leaves ~44 KiB
 * for the firmware (current blob is ~36 KiB).
 */
#define DDR_FIRMWARE_BASE	0xc0820000
