...
.. note::
   ...
   The procedure described here flashing images in SPL via USB fastboot.
   The fastboot function isn't enabled in SPL yet. So it requires a Spacemit
   Released SPL image.
...
.. code-block:: bash
    ...
    $cd {flash image path}
    $ln -sf {FSBL_PATH}/spl_bin/FSBL.bin ./build/
    $ln -sf {uboot build path}/u-boot.itb ./build/fit.itb
...
Apply the patch on the configuration files.
...
When ``flashserver`` is running, a fastboot device is expected to select. Just
