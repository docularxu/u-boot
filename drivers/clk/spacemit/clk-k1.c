...
/*
 * Each per-syscon clock driver spawns a UCLASS_RESET sibling on its own
 * ofnode via device_bind_driver_to_node() so that DT references of the
 * form `resets = <&syscon_apbc RESET_TWSI0>` resolve transparently.
 *
 * Pattern follows drivers/reset/reset-rockchip.c, called from each
 * Rockchip CRU clock driver. See drivers/reset/spacemit/reset-spacemit-k1.c
 * for the syscon-bound reset driver itself.
 */
