	...
	if (IS_ENABLED(CONFIG_MMC_IO_VOLTAGE)) {
		...
		case MMC_SIGNAL_VOLTAGE_180: {
			...
			if (IS_SD(mmc)) {
				...
				ok = !!(ctrl & SDHCI_CTRL_VDD_180) ==
				     to_180;
				if (!ok) {
					log_err("%d.%dV regulator output"
...
static int spacemit_sdhci_probe(struct udevice *dev)
{
	...
	struct dm_mmc_ops *mmc_driver_ops =
		(struct dm_mmc_ops *)dev->driver->ops;
	struct clk clk;
	int ret = 0;
	...
	ret = sdhci_setup_cfg(&plat->cfg, host, priv->clk_src_freq,
			      SDHC_MIN_CLOCK);
	if (ret)
		return ret;
...
static int spacemit_sdhci_of_to_plat(struct udevice *dev)
{
	...
	priv->clk_src_freq = dev_read_u32_default(dev, "clk-src-freq",
						  SDHC_DEFAULT_MAX_CLOCK);
