...
static void top_training_fp_all(u32 ddr_base, u32 cs_num, u32 boot_pp, void *input)
{
	...
	// The DDR training firmware is loaded in spl.c
	training = (void (*)(void * param))DDR_TRAINING_DATA_BASE;
	training(to_traning_param);
}

uint32_t lpddr4_silicon_init(struct ddr_config *cfg)
{
	u32 fp=0;
	...
	ddr_mid = SAMSUNG;
	top_Common_config(cfg->data_rate);

	if (!strcasecmp(cfg->type, "LPDDR4X"))
		io_para_update = io_para_select(LPDDR4X);
	else if (!strcasecmp(cfg->type, "LPDDR4"))
		io_para_update = io_para_select(LPDDR4);
	else
		io_para_update = &ddr_io_para_table[0];
	// convert resistance to ddr controller config value
	tx_odt_ohm = cfg->tx_odt;
	ddr_tx_odt = get_resistance_config(&tx_odt_ohm);
	printf("set ddr tx odt to %dohm!\n", tx_odt_ohm);

	top_DDR_MC_Phy_Device_Init((u32)(u64)cfg->base, cs_num, 0);

	top_training_fp_all((u32)(u64)cfg->base, cs_num, 0, info->para);

	size_mb = ddr_get_capacity(cfg);
	...
	adjust_mapping((u32)(u64)cfg->base, cs_num, size_mb, mr8_value);
	...
	fp=1;
	ddr_dfc(fp);
	top_training_fp_all((u32)(u64)cfg->base, cs_num, fp, info->para);

	fp=2;
	ddr_dfc(fp);
	top_training_fp_all((u32)(u64)cfg->base, cs_num, fp, info->para);
	if (16384 == size_mb)
		REG32(cfg->base + 0x24) = (0x10020095 | (3 << 24)); //bit7 MR21 RFU
