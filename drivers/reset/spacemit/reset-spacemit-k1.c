...
/*
 * SpacemiT K1 reset driver — syscon-bound (kernel-aligned binding).
 *
 * Bound by name from each per-syscon U_BOOT_DRIVER in the K1 clock
 * driver via device_bind_driver_to_node(). The reset device shares its
 * ofnode with the parent syscon clock device, so DT consumers using
 *   resets = <&syscon_apbc RESET_TWSI0>;
 * resolve transparently to the per-syscon UCLASS_RESET sibling.
 *
 * Reset signal IDs are taken from the kernel's
 * <dt-bindings/clock/spacemit,k1-syscon.h> (per-syscon namespace, each
 * syscon has its own [0..N) range). Per-syscon LUT contents mirror the
 * kernel-side reset driver layout.
 *
 * Pattern follows drivers/reset/reset-rockchip.c, used across ~10
 * Rockchip SoCs in mainline U-Boot.
 *
...
/* MPMU register offsets */
#define MPMU_WDTPCR			0x200

/* APBC register offsets */
...
static int spacemit_k1_reset_probe(struct udevice *dev)
{
	...
	if (!priv || !priv->base || !priv->table) {
		pr_err("spacemit_k1_reset: priv not initialised by parent\n");
		return -EINVAL;
	}
	return 0;
}
...
U_BOOT_DRIVER(spacemit_k1_reset) = {
	...
	/*
	 * No .of_match: bound by name from drivers/clk/spacemit/clk-k1.c
	 * via device_bind_driver_to_node(). Same pattern as
	 * drivers/reset/reset-rockchip.c (~10 Rockchip SoCs in mainline).
	 */
};
			   ...
			   enum spacemit_k1_reset_syscon syscon)
{
	...
	base = (void __iomem *)dev_remap_addr(parent);
	if (!base)
		return -ENODEV;
	...
	/*
	 * priv_auto can't be used here because the new device has no DT
	 * path of its own to read data from — populate priv before
	 * .probe() runs. Same approach as drivers/reset/reset-rockchip.c.
	 */
